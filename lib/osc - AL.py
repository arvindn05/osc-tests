...
##        ''' % (map_to_str(vc.ishttps, get_json_str_map()), vc.rabbitMQPort, vc.rabbitUser, vc.rabbitMQPassword)
    ...
    def createSFC(self, sfc):
        self._output.log_debug("Enter createSFC   - sfcname: \"%s\"  vcname: \"%s\"  " % (sfc.name, vc.name))
        sfcid = self.createSecFuncChain(sfcname=sfc.sfcname, vcname=sfc.vcname)

        self._output.log_debug("createSFC   - SFC Created - sfc ID: %s" % (sfcid))
    ...
    def deleteSecurityGroup(self, vcid, groupName):
        ...
        return self._wait_for_job(data, "JSON")
    ###AL:
    def deleteSerFuncChain(self, vcid, sfcname):
        self._output.log_debug("Enter deleteSeSerFuncChain\n -- VC: \"%s\"  SG: \"%s\"" %(vcid, sfcname))
        sfcid = self.getSerFuncChainID(vcid, sfcname)
        action = 'Delete serviceFunctionChain %s' % sfcid
        method = "DELETE"
        headers = 'JSON'
        body = ''
        delUrl = '/api/server/v1/virtualizationConnectors/%s/serviceFunctionChain/%s' % (vcid, sfcid)
        url = delUrl
        self._output.log_debug("deleteSerFuncChain\n -- Sending %s Request for Action: %s:\n -- IP Addr: \"%s\"\n -- Headers: \"%s\"\n -- Method: \"%s\"\n -- URL: \"%s\"\n\n -- Body:\n%s" %(method, action, self.iscaddr, headers, method, url, body))
        data = self._isc_connection('DELETE', delUrl, '', action, "JSON")
        return self._wait_for_job(data, "JSON")
...
##        self._output.log_debug("addSecurityGroupBindingViaVirtSys -- Returned from: 'createSecGrpInterfaceViaVirtSys'")
    ...
    pass
    ###AL:
    def getSerFuncChainIdListByVcId(self, vc_id):
        url = "/api/server/v1/virtualizationConnectors/%s/serviceFunctionChain" %(vc_id)
        self._output.log_debug("getSerFuncChainIdListByVcId -- calling 'getQueryDict'")
        sfc_dict = self.getQueryDict(url)
        self._output.log_debug("getSerFuncChainIdListByVcId -- VCId: %s   Returning SFC Ids:\n%s" %(vc_id, self._output.pformat(sfc_ids)))
        return(sfc_ids)
    pass
