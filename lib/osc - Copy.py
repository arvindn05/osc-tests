...
from ostack_support import getOstackCred, getOstackSession, get_networks, get_subnets, get_projects, instance_list, network_list, subnet_list, project_list
    ...
    def _wait_for_job(self, data, headers="JSON"):
        ...
        while (not jobFound) and (cnt <= 4):
            ...
            try:
                ...
                val = self._findInXMLOrJSON(data=data, key="state")
                jobFound = True
            except:
                pass
            pass
        ...
        ## print("\n\nCnt: %s   jobId: \"%s\"\n -- Data:\n%s\n\n" %(cnt, jobId, data))
        # Rather than have a hard timeout here, we will allow QAET to time
        # out the script if it runs too long.
        # (State can be NOT RUNNING, QUEUED, RUNNING, or COMPLETED)
        try:
...
##        ''' % (map_to_str(vc.ishttps, get_json_str_map()), vc.rabbitMQPort, vc.rabbitUser, vc.rabbitMQPassword)
        ...
        vcnames = con_dict.keys()
        vcids = con_dict.values()
        self._output.log_debug("createOrUpdateOstackVC\n\n -- vcname(type=%s): \"%s\"\n\n -- con_dict(type=%s):\n%s\n\n -- con_dict.values():\n%s\n\n -- con_dict.keys():\n%s" %(type(vc.name), self._output.pformat(vc.name), type(con_dict), self._output.pformat(con_dict), self._output.pformat(list(con_dict.values())), self._output.pformat(list(con_dict.keys()))))
        self._output.log_debug("Con Dict Keys(type=%s):\n\n%s" %(type(con_dict_keys), self._output.pformat(con_dict_keys)))

        if update_vcid:
            if update_vcid not in vcids:
                self._output.log_error("createOrUpdateOstackVC -- VC to update 'update_vcid' \"%s\" not found" %(update_vcid))
            pass
        elif vc.name in con_dict_keys:
            self._output.log_debug("createOrUpdateOStackVC -- Virtualization Connector %s already exists at ID %s" % (vc.name, con_dict[vc.name]))
            if (vc.name, vc.providerIP, vc.providerUser, keystoneip, keystoneuser, "OPENSTACK",
                "Juno") == self.getVirtualizationConnectorbyID(con_dict[vc.name]):
                self._output.log_debug("Same values, so using")
                return con_dict[vc.name]
            else:
                self._output.log_debug("Different values, not modifying")
                self._output.log_debug("Existing values:")
                for val in (vc.name, keystoneip, keystoneuser, keystoneip, keystoneuser, "OPENSTACK", "Juno"):
                    self._output.log_debug("\t%s" % val)
                self._output.log_debug("Requested values:")
                for val in self.getVirtualizationConnectorbyID(con_dict[vc.name]):
                    self._output.log_debug("\t%s" % val)
                message = "ISC %s can not modify an existing object" % action
                raise ISCOperationalError(message)
            pass
        pass
    ...
    def updateVC(self, vc, update_vcid=None):
        if (not update_vcid):
            self._output.log_error("updateVC -- No 'update_vcid' arg given, and 'vc' has no 'update_vcid' attribute -- VC:\n%s" %(self._output.pformat(vc_dict)))
        pass
    ...
    def createOrUpdateManagerConnector(self, mcname=None, mgrip=None, mgruser=None, mgrpasswd=None, mgrkey=None,
        ...
        '''
        con_dict = self.getManagerConnectors()
        ##if con_dict and (mcname in con_dict.keys()):
        if False:
            errmsg = "Manager Connector %s already exists at ID %s" % (mcname, con_dict[mcname])
            # self._output.log_warn(errmsg)
            self._output.log_error(errmsg, eclass=ISCOperationalError)
            #
            # if (mcname, "mgr", mgrip) == self.getManagerConnectorById(con_dict[mcname]):
            #     self._output.log_error(errmsg, eclass=ISCOperationalError)
            #     self._output.log_debug("Same values, so using")
            #     return con_dict[mcname]
            # else:
            #     self._output.log_debug("Different values, not modifying")
            #     self._output.log_debug("Existing values:")
            #     for val in (mcname, "mgr", mgrip):
            #         self._output.log_debug("\t%s" % val)
            #     self._output.log_debug("Requested values:")
            #     for val in self.getManagerConnectorById(con_dict[mcname]):
            #         self._output.log_debug("\t%s" %(val))
            #     pass
            #     errmsg = "ISC %s can not modify an existing object" %(action)
            #     ##raise ISCOperationalError(errmsg)
            #     self._output.log_error(errmsg, eclass=ISCOperationalError)
            # pass
            #
        '''
        ...
        new_mcid = self._findIDinJSON(data)

        self._output.log_debug("NEW_MCID: \"%s\"" % (new_mcid))
        return new_mcid
    ...
    def _createDistributedAppliance(self, daname, mcname, mcid, apid, model, swver, vcid, vcname, vctype, domainName,
        ...
        '''
        con_dict = self.getDistributedAppliances()
        if daname in con_dict.keys():
            self._output.log_debug("Distributed Appliance %s already exists at ID %s" % (daname, con_dict[daname]))
            if (daname, mcname, vcname, "OPENSTACK", model, swver, "Shared Domain") == self.getDistributedAppliancebyID(
                    con_dict[daname]):
                self._output.log_debug("Same values, so using")
                return con_dict[daname]
            else:
                self._output.log_debug("Different values, not modifying")
                self._output.log_debug("Existing values:")
                for val in (daname, mcname, vcname, "OPENSTACK", model, swver, "Shared Domain"):
                    self._output.log_debug("\t%s" % val)
                self._output.log_debug("Requested values:")
                for val in self.getDistributedAppliancebyID(con_dict[daname]):
                    self._output.log_debug("\t%s" % val)
                return 0
        '''
    ...
    def _deployOStackAppliance(self, dsname=None, daname=None, projectName=None, region=None, mgmtNetName=None, inspNetName=None, ippool=None, count=1, shared=True, update=False):
        ...
        project_nm_to_id = get_projects(session=ostk_session)
    ...
    ## def __init__(self, sg_name, vc_name, project_name, protect_all, encode_unicode=False):
        ...
        if update_name_or_id:
            ...
            if hasattr(sg_obj, 'project_name') and getattr(sg_obj, 'project_name') and (str(getattr(sg_obj, 'project_name')).lower() != 'none'):
                project_name = getattr(sg_obj, 'project_name')
        ...
        ostk_session = getOstackSession(auth_ip=vc_ip)
        self._output.log_debug("_createOrUpdateSG -- Ostack Session: %s\n%s" %(ostk_session, self._output.objformat(ostk_session)))
        project_nm_to_id = get_projects(session=ostk_session)
        ...
        ##self._output.log_debug("_createOrUpdateSG\n -- Sending %s Request for Action: %s:\n -- IP Addr: \"%s\"\n -- Headers: \"%s\"\n -- Method: \"%s\"\n -- URL: \"%s\"\n\n -- Body:\n%s" %(method, action, self.iscaddr, headers, method, url, body))
...
#        pass
    ...
    def _bindPolicy(self, vcid, da_id, sgid, daname, policyId, failurePolicy, order, is_binded):
        ...
        ##print("\n\nBind Policy -- URL: \"%s\"\n\nBody:\n%s\n\n" %(url, body))
        uself._output.log_debug("_bindPolicy\n -- Sending %s Request for Action: %s:\n -- IP Addr: \"%s\"\n -- Headers: \"%s\"\n -- Method: \"%s\"\n -- URL: \"%s\"\n\n -- Body:\n%s" %(method, action, self.iscaddr, headers, method, url, body))
...
##############################
    ...
    def addSecurityGroupMemberObj(self, sg_mbr, ostack_cred=None):
        ...
        if not ostack_cred:
            ostack_cred = {'auth_ip':ostack_ip, 'project_name':'demo'}
